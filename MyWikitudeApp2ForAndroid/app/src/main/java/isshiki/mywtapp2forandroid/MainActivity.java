package isshiki.mywtapp2forandroid;
...
public class MainActivity extends AppCompatActivity {
	...
	//　Android：パッケージ名（applicationId）：isshiki.mywtapp2forandroid
	...
	/** JavaScriptからObjective-C側への通信を受信するためのリスナー（※本サンプルではコードはありますが実質的には使っていません）。*/
	protected ArchitectJavaScriptInterfaceListener jsonListener;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// SDKキーと必要な機能（ジオロケーションと画像トラッキング）を指定します。
		final ArchitectStartupConfiguration config = new ArchitectStartupConfiguration();
		config.setLicenseKey(WIKITUDE_SDK_KEY);
		config.setCameraPosition(CameraPosition.DEFAULT);
		config.setCameraResolution(CameraResolution.AUTO);
		config.setCameraFocusMode(CameraFocusMode.CONTINUOUS);
		config.setCamera2Enabled(false);        // camera2 API を無効化して古い camera API を使う。
		config.setFeatures(ArchitectStartupConfiguration.Features.Geo | ArchitectStartupConfiguration.Features.ImageTracking);    // 物体トラッキング用の「ObjectTracking」や、インスタントトラッキング用の「InstantTracking」という値もある
		...
		// ARchitect World用URLリスナーのオブジェクトを作成します。JavaScript上で「AR.platform.sendJSONObject」メソッドにより呼び出されたJSONデータはこのリスナーに通知されます。これを利用することで、 「JavaScriptコード」と「ネイティブAndroidアクティビティ／フラグメント」の間でのやり取りが実現できます。
		this.jsonListener = this.getJsonListener();
		// ※ARchitectViewへのARchitect World用URLリスナーの登録を行います。イベントの受け取り漏れを無くすために、コンテンツがロードされる前のここで必ず登録するようにしてください。
		if (this.jsonListener != null && this.architectView != null) {
			this.architectView.addArchitectJavaScriptInterfaceListener(this.getJsonListener());
		}
	...
	// region ARchitect World用
	//
	// // 　（2）JavaScriptからObjective-C側への通信で使われる「AR.platform.sendJSONObject」によるJSONデータを受け取る「receivedJSONObject」メソッドの実装
	// 「architectsdk://」プロトコル経由のURL呼び出しの管理（※本サンプルではコードはありますが実質的には使っていません）

	/**
	 *
	 * @return ARchitectView用のJSON通信リスナー。
	 */
	public ArchitectJavaScriptInterfaceListener getJsonListener() {
		return new ArchitectJavaScriptInterfaceListener() {

			@Override
			public void onJSONObjectReceived(JSONObject jsonObject) {
				try {
					// JSONオブジェクトの内容をチェックして、ボタンごとに動作を振り分けたりできます。// 例えば「AR.platform.sendJSONObject({ action: "capture_screen" });」というコードで呼び出されているかどうかなど。
					switch (jsonObject.getString("action")) {
						case "captureScreen":
							// ［スナップショット］ボタンが押されたときの処理。
					...
					MainActivity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							Toast.makeText(MainActivity.this, R.string.error_parsing_json, Toast.LENGTH_LONG).show();
						}
					});
					e.printStackTrace();
				}
